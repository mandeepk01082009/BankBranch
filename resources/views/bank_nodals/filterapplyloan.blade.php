...
@section('content')
    ...
    <div class="container-fluid pt-4 px-4">
        <div class="row g-4">
            <div class="col-sm-12 col-xl-12">
                ...
                <form id="bankNodalFilterForm" action="{{ route('filterapplyloan') }}" method="GET">
                    <div class="row">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="created_at" id="bankNodallast_chat_date" class="form-control" value="{{ request()->query('created_at') }}">
                        </div>
                        <div class="col-md-3">
                            <label for="apply_date">Apply date:</label>
                            <input type="date" name="apply_date" id="bankNodalapply_date" class="form-control" value="{{ request()->query('apply_date') }}">
                        </div>
                        <!-- <div class="col-md-3">
                        ...
                        <div class="col-md-2 mt-4">
                            ...
                            <button type="reset" id="bankNodalresetBtn" class="btn btn-danger">Reset</button>
                        </div>
                    </div>
                </form>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($applyloan as $applicant)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('messageSend', ['applicantId' => $applicant->id]) }}" class="btn btn-success">Chat</a>
                                            </td>
                                        </tr>
    ...
    <script type="text/javascript">
        ...
        function updateResetButton() {
            var isEmpty = $('#bankNodalstatus').val() == '' && $('#bankNodallast_chat_date').val() == '' && $('#bankNodalapply_date').val() == '';
            $('#bankNodalresetBtn').prop('disabled', isEmpty);
        }
    
        // Listen for changes in the form fields
        $('#bankNodalFilterForm').on('change input', 'input, select', function() {
            // Automatically submit the form
            $('#bankNodalFilterForm').submit();
            // Update the disabled state of the reset button
            updateResetButton();
        });
    
        // Listen for click on the reset button
        $('#bankNodalresetBtn').click(function() {
            // Set the status field to "All"
            $('#bankNodalstatus').val('');
    
            // Clear other field values
            $('#bankNodallast_chat_date').val('');
            $('#bankNodalapply_date').val('');
