        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-12">
                <div class="bg-light rounded h-100 p-4">
                    ...
                    <form id="bankBranchFilterForm" action="{{ route('filter-apply-loan') }}" method="GET">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <select name="status" id="bankBranchstatus" class="form-select">
                                    ...
                                    <option value="Send back to user"
                                        {{ request()->query('status') == 'Send back to user' ? 'selected' : '' }}>Send back
                                        to user</option>
                                    <option value="Approved"
                                        {{ request()->query('status') == 'Approved' ? 'selected' : '' }}>Approved</option>
                                    <option value="Rejected"
                                        {{ request()->query('status') == 'Rejected' ? 'selected' : '' }}>Rejected</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label for="last_chat_date">Last chat date:</label>
                                <input type="date" name="created_at" id="bankBranchlast_chat_date" class="form-control"
                                    value="{{ request()->query('created_at') }}">
                            </div>
                            <div class="col-md-3">
                                <label for="apply_date">Apply date:</label>
                                <input type="date" name="apply_date" id="bankBranchapply_date" class="form-control"
                                    value="{{ request()->query('apply_date') }}">
                            </div>
                            <!-- <div class="col-md-3">
                            ...
                            <div class="col-md-2 mt-4">
                                ...
                                <button type="reset" id="bankBranchresetBtn" class="btn btn-danger">Reset</button>
                            </div>
                        </div>
                    </form>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($applyloan as $applicant)
                                            <tr>
                                                ...
                                                <td>{{ $applicant->created_at->timezone('Asia/Kolkata')->format('M d, Y h:i A') }}
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <form
                                                        action="{{ route('applicantView', ['applicant' => $applicant->id]) }}"
                                                        method="GET">
                                                        @csrf
                                                ...
                                                <td>
                                                    <a href="{{ route('messagesSend', ['applicantId' => $applicant->id]) }}"
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function updateResetButton() {
                var isEmpty = $('#bankBranchstatus').val() == '' && $('#bankBranchlast_chat_date').val() == '' && $(
                    '#bankBranchapply_date').val() == '';
                $('#bankBranchresetBtn').prop('disabled', isEmpty);
            }

            // Listen for changes in the form fields
            $('#bankBranchFilterForm').on('change input', 'input, select', function() {
                // Automatically submit the form
                $('#bankBranchFilterForm').submit();
                // Update the disabled state of the reset button
                updateResetButton();
            });

            // Listen for click on the reset button
            $('#bankBranchresetBtn').click(function() {
                // Set the status field to "All"
                $('#bankBranchstatus').val('');

                // Clear other field values
                $('#bankBranchlast_chat_date').val('');
                $('#bankBranchapply_date').val('');
