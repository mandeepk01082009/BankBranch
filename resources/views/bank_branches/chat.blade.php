...
<body class="bg-light">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">Chat
                        <a href="{{ route('filter-apply-loan') }}"><button type="button" class="btn btn-primary"
                    ...
                    <div class="card-body">
                        ...
                        @if (!$hasApprovedOrRejectedMessage)
                            <form id="bankBranch-message-form" method="POST"
                                action="{{ route('messagesStore', $applicantId) }}" enctype="multipart/form-data">
                                @csrf

                                <div class="form-group">
                                    <textarea id="bankBranch-message" name="text" class="form-control" rows="3"
                                ...
                                <div class="form-group mt-3">
                                    <input type="file" id="bankBranch-file" name="file[]" class="form-control"
                                        multiple>
                                    @error('file')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                ...
                                <div class="invalid-feedback" id="bankBranch-field-error" style="display: none;">
                                    <strong> Please fill in
                                        either the text or the file field.</strong>
                                </div>
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    @enderror
                                    <div id="bankBranch-status-error" class="invalid-feedback" style="display: none;">
    ...
    <script>
        document.getElementById('bankBranch-message-form').addEventListener('submit', function(event) {
            var status = document.getElementById('bankBranch-status').value.trim();
            var text = document.getElementById('bankBranch-message').value.trim();
            var file = document.getElementById('bankBranch-file').value.trim();

            if (!status || (!text && !file)) {
                event.preventDefault();

                if (!status) {
                    document.getElementById('bankBranch-status-error').style.display = 'block';
                }

                if (!text && !file) {
                    document.getElementById('bankBranch-field-error').style.display = 'block';
                }
            } else {
                document.getElementById('bankBranch-status-error').style.display = 'none';
                document.getElementById('bankBranch-field-error').style.display = 'none';
            }
        });
    </script>
