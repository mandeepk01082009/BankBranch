        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-12">
                <div class="bg-light rounded h-100 p-4">
                    <a href="{{ route('add_slider') }}"><button type="button" class="btn btn-primary" style="float: right;">Add</button></a>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($slider as $sliders)
                                    <tr>
                                        {{-- <th scope="row">1</th> --}}
                                        {{-- <td>{{ $sliders->sort_col }}</td> --}}
                                        <td>{{ $sliders->title }}</td>
                                        <td>{{ $sliders->description }}</td>
                                        <td><img src="{{ asset('storage/' . $sliders->image) }}" style="max-width: 200px; max-height:200px"></td>
                                        <td>{{ $sliders->status == 1 ? 'Active' : ''  }}</td>
                                        <td>{{ $sliders->created_at }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('edit_slider', $sliders->id) }}"
                                            ...
                                            <form method="POST"
                                                action="{{ route('delete_slider', $sliders->id) }}">
                                                @csrf
                                                @method('delete')
